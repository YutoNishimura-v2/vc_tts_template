...
def Conv1d(in_channels, out_channels, kernel_size, *args, **kwargs):
    ...
    m = conv.Conv1d(in_channels, out_channels, kernel_size, *args, **kwargs)  # bufferつき.
    return nn.utils.weight_norm(m)  # weight normalization.
...
class ResSkipBlock(nn.Module):
    ...
    def clear_buffer(self):  # wavenet本体で利用されていた.
        """Clear input buffer."""
        for c in [
            self.conv,
            self.conv1x1_out,
