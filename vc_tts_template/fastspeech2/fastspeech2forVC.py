...
from vc_tts_template.fastspeech2.encoder_decoder import Encoder
from vc_tts_template.fastspeech2VC.encoder_decoder import Decoder
...
class FastSpeech2ForVC(nn.Module):
    """ FastSpeech2ForVC
    Voice Transformer Network: Sequence-to-Sequence Voice Conversion Using Transformer with Text-to-Speech Pretraining
    ↑これの実現の為に, VCと同様の名前、構造をもったdecoderを用意する.
    """
    ...
    def __init__(
        ...
        super(FastSpeech2ForVC, self).__init__()
    ...
    def decoder_forward(
        ...
        if self.emotion_emb is not None:
            output = output + self.emotion_emb(em_ids).unsqueeze(1).expand(-1, output.size(1), -1)

        output = self.decoder(self.decoder_linear(output), mel_masks)
        output = self.mel_linear_2(output).contiguous().view(output.size(0), -1, self.mel_num)

        postnet_output = self.postnet(output) + output

        mel_lens = mel_lens * self.reduction_factor
        mel_masks = make_pad_mask(mel_lens, torch.max(mel_lens).item())
    ...
    def forward(
        ...
        output, postnet_output, mel_lens, mel_masks = self.decoder_forward(
            output, speakers, emotions, mel_lens, mel_masks
        )
